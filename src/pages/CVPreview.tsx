...
export function CVPreview({ onBack }: { onBack: () => void }) {
  ...
  const sendToWhatsApp = () => {
    ...
    const message = `*--- NEW CV ORDER ---*%0A%0A*Name:* ${cvData.personalInfo.name}%0A*Amount:* Rs. 100.00%0A%0AI have paid and attached the slip. Please send my PDF CV.%0A%0A*System Ref:* ${encodedRef}`;
    ...
    <div className="min-h-screen bg-gray-100 pb-20">
      <div className="bg-white border-b sticky top-0 z-40 px-4 py-3 shadow-sm flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button onClick={() => setShowThemeSelector(true)} className="p-2 bg-gray-100 rounded-full"><PaletteIcon size={20} /></button>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
          <div className="bg-white rounded-3xl max-w-md w-full p-8 shadow-2xl">
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={() => setShowPaymentModal(false)}><XIcon className="w-6 h-6" /></button>
            </div>
            ...
            <label className="block p-8 border-2 border-dashed border-gray-200 rounded-2xl text-center cursor-pointer mb-6 hover:bg-zinc-50">
              {slipImage ? <img src={slipImage} className="h-40 mx-auto rounded-lg" alt="Slip" /> : <div className="text-gray-400 font-bold"><CameraIcon size={30} className="mx-auto mb-2"/> UPLOAD SLIP</div>}
            ...
            <button 
              ...
              className="w-full py-5 bg-[#25D366] text-white rounded-2xl font-black shadow-xl flex items-center justify-center gap-2"
            >
      ...
      {/* Theme Selector Modal (එකම විදිහට XIcon tags ටික හදාගන්න) */}
      {showThemeSelector && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="bg-white rounded-3xl max-w-2xl w-full p-8 max-h-[85vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-8">
              ...
              <button onClick={() => setShowThemeSelector(false)}><XIcon className="w-6 h-6" /></button>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-6">
              {templateThemes.map((theme) => (
                <button
                  ...
                  className={`p-2 rounded-2xl border-4 ${cvData.selectedTemplate === theme.id ? 'border-blue-500 scale-105' : 'border-gray-50'}`}
                >
