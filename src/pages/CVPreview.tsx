...
export function CVPreview({ onBack }: { onBack: () => void }) {
  ...
  return (
    <div className="h-screen bg-zinc-950 flex overflow-hidden relative">
      ...
      {/* --- Sidebar: Controls --- */}
      <div className={`fixed md:relative z-[70] h-full bg-white border-r w-[320px] md:w-[380px] transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-6 bg-zinc-50/30 custom-scrollbar">
          {/* Template Styles */}
          <div>
            <p className="text-[10px] font-black uppercase text-zinc-400 mb-3 ml-1 tracking-widest">Template Style</p>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <p className="text-[10px] font-black uppercase text-zinc-400 mb-2">Theme Color</p>
            ...
            <div>
              <p className="text-[10px] font-black uppercase text-zinc-400 mb-2">Preview Zoom</p>
      ...
      <div className="flex-1 relative flex flex-col h-full overflow-hidden bg-zinc-900">
        ...
        <div className="flex-1 overflow-auto p-4 md:p-12 flex justify-center items-start custom-scrollbar">
          {/* මෙතනදී තමයි CV එක පේන්නේ. cvData එකේ image එක හරියට යන්න ඕනේ */}
          <div className="transition-all duration-500 shadow-2xl origin-top" style={{ transform: `scale(${previewScale})` }}>
      ...
      {showPayment && (
        <div className="fixed inset-0 bg-black/90 z-[100] flex items-center justify-center p-4 backdrop-blur-md">
          <div className="bg-white rounded-[2.5rem] p-8 max-w-md w-full shadow-2xl relative">
            ...
            <h3 className="text-2xl font-black uppercase mb-6 italic">Checkout</h3>
