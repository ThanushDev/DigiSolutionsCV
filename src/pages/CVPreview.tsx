...
export function CVPreview({ onBack }: { onBack: () => void }) {
  ...
  // --- Payment Slip Upload Logic (ImgBB) ---
  ...
  // --- WhatsApp Ref Logic (JSON Data) ---
  const handleWhatsApp = () => {
    if (!slipUrl) return alert("Please upload the payment slip first!");
    
    // මුළු CV Data එකම JSON කරලා Encode කරනවා (මේකෙ තමයි photo URL එකත් තියෙන්නෙ)
    ...
    <div className="h-screen bg-zinc-950 flex overflow-hidden relative font-sans">
      ...
      <div className={`fixed md:relative z-[70] h-full bg-white border-r w-[320px] md:w-[380px] transition-transform duration-300 flex flex-col ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-6 bg-zinc-50/30 custom-scrollbar">
          ...
          <div className="space-y-3">
            <p className="text-[10px] font-black uppercase text-zinc-400 ml-1 tracking-widest">Select CV Template</p>
            <div className="grid grid-cols-1 gap-2">
              {templateThemes.map((t) => (
                <button 
                  key={t.id} 
                  onClick={() => { setSelectedTemplate(t.id); updateThemeColor(t.primaryColor); }}
                  className={`p-4 rounded-2xl text-left border-2 transition-all flex justify-between items-center ${cvData.selectedTemplate === t.id ? 'border-blue-600 bg-blue-50' : 'bg-white border-transparent shadow-sm hover:border-zinc-200'}`}
                >
      ...
      <div className="flex-1 relative flex flex-col h-full bg-zinc-900 overflow-hidden">
        ...
        <div className="md:hidden p-4 flex justify-between items-center z-30 bg-zinc-900/50 backdrop-blur-md">
          <button onClick={() => setIsSidebarOpen(true)} className="p-3 bg-white/10 rounded-xl text-white border border-white/10 hover:bg-white/20 transition-all"><Menu size={20}/></button>
          <span className="text-white font-black uppercase text-[10px] tracking-widest opacity-60">CV Preview</span>
        ...
        <div className="flex-1 overflow-auto p-4 md:p-12 flex justify-center items-start custom-scrollbar">
          <div className="transition-all duration-500 shadow-[0_40px_100px_rgba(0,0,0,0.6)] origin-top bg-white" style={{ transform: `scale(${previewScale})` }}>
            {/* PhotoUpload එකෙන් ලැබෙන cvData.personalInfo.photo එක මෙතන auto display වෙනවා */}
      ...
      {showPayment && (
        <div className="fixed inset-0 bg-black/95 z-[100] flex items-center justify-center p-4 backdrop-blur-xl animate-in fade-in duration-300">
          <div className="bg-white rounded-[2.5rem] p-8 max-w-md w-full shadow-2xl relative animate-in zoom-in duration-300">
            ...
            <p className="text-zinc-400 text-[10px] mb-6 font-bold uppercase tracking-widest text-center italic">Pay Rs. 500 to process your order</p>
            
            <div className="bg-zinc-950 text-white rounded-[2.5rem] p-7 mb-6 shadow-2xl">
              <div className="space-y-4">
                <div>
                  <p className="text-[9px] uppercase font-black text-zinc-500 tracking-[0.2em]">Bank (BOC)</p>
                  <p className="text-sm font-bold text-zinc-300">Bank of Ceylon</p>
                </div>
                <div>
                  <p className="text-2xl text-blue-400 font-black tracking-wider underline">91691764</p>
                </div>
                <div>
                  <p className="text-[9px] uppercase font-black text-zinc-500 tracking-[0.2em]">Account Holder</p>
                  <p className="text-sm font-bold uppercase tracking-tight text-zinc-100">W. S. SEEHARA</p>
                </div>
              </div>
            </div>
