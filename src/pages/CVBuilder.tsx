...
import { Extracurriculars } from '../components/FormSections/Extracurriculars';
...
export function CVBuilder({ onPreview }: { onPreview: () => void }) {
  ...
  const handleReset = () => {
    if (window.confirm("Are You Sure, Your Data will be Lost!")) {
      resetCV();
      setCurrentStep(1);
    }
  };
  ...
  const renderStep = () => {
    switch (currentStep) {
      ...
      case 9: return <Achievements />;
      case 10: return <Extracurriculars />;
  ...
  const steps = [
    ...
    { n: 4, label: 'Languages' }, { n: 5, label: 'Work Experience' }, { n: 6, label: 'Education' },
    { n: 7, label: 'Qualifications' }, { n: 8, label: 'Projects' }, { n: 9, label: 'Achievements' },
    { n: 10, label: 'Activities' }, { n: 11, label: 'References' }
  ];
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-10">
      ...
      <div className="flex justify-between mb-8 overflow-x-auto pb-2 gap-4 scrollbar-hide">
        {steps.map((s) => (
          <button key={s.n} onClick={() => setCurrentStep(s.n)} className={`flex-1 min-w-[80px] pb-2 border-b-4 transition-all text-[10px] font-black uppercase whitespace-nowrap ${currentStep === s.n ? 'border-blue-600 text-blue-600' : 'border-gray-100 text-gray-400'}`}>
            {s.label}
          </button>
        ))}
      </div>
      ...
      <div className="bg-white rounded-[2.5rem] shadow-2xl shadow-blue-100/50 p-6 md:p-12 border border-gray-50">
        ...
        <div className="mt-12 flex justify-between items-center border-t pt-8">
          ...
          <button 
            onClick={() => currentStep === 11 ? onPreview() : setCurrentStep(currentStep + 1)} 
          ...
          >
            {currentStep === 11 ? 'Finish & Preview' : 'Next Step →'}
          </button>
        </div>
      </div>
    </div>
