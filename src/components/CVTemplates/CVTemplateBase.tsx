...
const DateSignature = ({ show }: { show: boolean }) => {
  ...
  return (
    <div className="mt-auto flex justify-between items-end w-full pb-8 px-8 flex-shrink-0">
      ...
      <div className="text-center">
        <div className="w-32 border-b-2 border-zinc-400 border-dotted mb-3"></div>
            ...
            <div className="w-[32%] bg-zinc-50 border-r h-full p-8 flex flex-col gap-6 shrink-0 text-zinc-600">
              ...
              <section>
                ...
                <div className="space-y-3 text-[10px] font-bold">
                  ...
                  <p className="flex items-start gap-2 whitespace-pre-wrap leading-relaxed"><MapPin size={12} className="shrink-0 mt-0.5"/> {address}</p>
                </div>
              </section>
        ...
        return (
          <div className="flex flex-col h-full">
            <div className="h-32 flex items-center justify-between px-10 text-white shrink-0" style={{ backgroundColor: theme }}>
              <div className="max-w-[70%]"><h1 className="text-3xl font-black uppercase tracking-tight">{fullName}</h1><p className="text-[10px] opacity-80 uppercase tracking-widest mt-2 flex items-center gap-3"><span className="flex items-center gap-1"><Mail size={10}/> {email}</span> | <span className="flex items-center gap-1"><Phone size={10}/> {phone1}</span></p></div>
            ...
            <div className="flex flex-1 p-10 gap-10 overflow-hidden">
              <div className="w-[28%] space-y-6 shrink-0 border-r pr-6">
                <section>
                  <SectionTitle title="Contact" />
                  <div className="text-[10px] font-bold text-zinc-500 space-y-3">
                    <p className="flex items-start gap-2 whitespace-pre-wrap leading-relaxed"><MapPin size={12} className="shrink-0 mt-0.5"/> {address}</p>
                    <p className="flex items-center gap-2"><Phone size={12}/> {phone1}</p>
                  </div>
                </section>
        ...
        return (
          <div className="flex flex-col h-full p-10">
            <div className="relative h-28 bg-zinc-900 rounded-[2.5rem] flex items-center px-10 mb-10 shrink-0">
              <div className="max-w-[60%]"><h1 className="text-2xl font-black text-white uppercase italic leading-none">{fullName}</h1><p className="text-[10px] text-zinc-400 mt-2 uppercase tracking-widest flex items-center gap-2"><Mail size={10}/> {email}</p></div>
            ...
            <div className="grid grid-cols-12 gap-8 flex-1">
              <div className="col-span-4 space-y-6">
                <div className="p-8 rounded-[2.5rem] bg-zinc-50 border border-zinc-100 space-y-8">
                  <section>
                    <p className="text-[11px] font-black uppercase mb-3 flex items-center gap-2" style={{ color: theme }}>Contact Info</p>
                    <div className="text-[10px] text-zinc-500 font-bold space-y-2.5 leading-relaxed">
                      <p className="flex items-start gap-2 whitespace-pre-wrap"><MapPin size={12} className="shrink-0 mt-0.5"/> {address}</p>
                      <p className="flex items-center gap-2"><Phone size={12}/> {phone1}</p>
                    </div>
                  </section>
        ...
        return (
          <div className="flex h-full text-zinc-300">
            <div className="w-[32%] bg-zinc-900 p-8 flex flex-col gap-6 shrink-0">
              ...
              <section>
                <h3 className="border-b border-zinc-700 pb-2 mb-4 text-[10px] font-black uppercase text-zinc-400">Contact</h3>
          ...
          <div className="flex flex-col h-full p-12 text-zinc-800">
            <div className="flex justify-between items-start border-b-[4px] pb-8 mb-8 shrink-0" style={{ borderColor: theme }}>
              <div className="max-w-[75%]">
                <h1 className="text-3xl font-black uppercase mb-2 tracking-tight">{fullName}</h1>
                <div className="text-[10.5px] font-bold text-zinc-500 flex flex-wrap gap-x-4 gap-y-1">
                  <span className="flex items-center gap-1"><Phone size={10}/> {phone1}</span>
                  <span className="flex items-center gap-1"><Mail size={10}/> {email}</span>
                  <span className="flex items-center gap-1"><MapPin size={10}/> {address.replace(/\n/g, ', ')}</span>
                </div>
              </div>
              ...
              <div className="w-52 bg-zinc-50 p-8 rounded-[2.5rem] h-fit space-y-8 shrink-0 border border-zinc-100">
                ...
                <section>
                  ...
                  <p className="text-[9.5px] font-bold text-zinc-500 whitespace-pre-wrap leading-relaxed flex items-start gap-2">
                    <MapPin size={12} className="shrink-0 mt-0.5"/> {address}
                  </p>
                </section>
