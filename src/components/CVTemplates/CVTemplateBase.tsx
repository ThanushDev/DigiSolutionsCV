...
export function CVTemplateBase({ cvData, scale = 1 }: { cvData: CVData; scale?: number }) {
  ...
  const PersonalInfoList = ({ isDark = false }) => (
    <div className={`space-y-1.5 text-[10px] font-bold uppercase ${isDark ? 'text-zinc-400' : 'text-zinc-600'}`}>
      {nic && <p className="grid grid-cols-[70px_1fr]"><span className="opacity-60 text-[9px]">NIC:</span> <span>{nic}</span></p>}
      {dob && <p className="grid grid-cols-[70px_1fr]"><span className="opacity-60 text-[9px]">DOB:</span> <span>{dob}</span></p>}
      {gender && <p className="grid grid-cols-[70px_1fr]"><span className="opacity-60 text-[9px]">Gender:</span> <span>{gender}</span></p>}
      {nationality && <p className="grid grid-cols-[70px_1fr]"><span className="opacity-60 text-[9px]">Nationality:</span> <span>{nationality}</span></p>}
      {religion && <p className="grid grid-cols-[70px_1fr]"><span className="opacity-60 text-[9px]">Religion:</span> <span>{religion}</span></p>}
      {civilStatus && <p className="grid grid-cols-[70px_1fr]"><span className="opacity-60 text-[9px]">Civil Status:</span> <span>{civilStatus}</span></p>}
    </div>
  );
    ...
    <div className="flex h-full min-h-[1120px] bg-white text-[11px]">
      <div className="w-[33%] bg-zinc-50 border-r p-8 flex flex-col gap-8">
        <div className="flex flex-col items-center">
          ...
          <section className="w-full text-left mt-8">
            ...
            <div className="space-y-3 pl-1 text-zinc-600 text-[10px]">
              <div className="flex items-center gap-2"><Phone size={12} style={{ color: theme }}/> {phone1}</div>
          ...
          <section className="w-full text-left mt-6">
            ...
            <div className="flex flex-wrap gap-2 pl-1">{skillsList.map((s, i) => <span key={i} className="bg-white border px-2 py-1 rounded text-[9px] font-bold uppercase shadow-sm">{s}</span>)}</div>
          </section>
          <section className="w-full text-left mt-6">
            ...
            <div className="flex flex-wrap gap-2 pl-1">{langList.map((l, i) => <span key={i} className="bg-white border px-2 py-1 rounded text-[9px] font-bold uppercase shadow-sm">{l}</span>)}</div>
          </section>
        </div>
      </div>
      <div className="flex-1 p-10 flex flex-col space-y-6">
        <div>
          ...
          {summary && <p className="text-zinc-500 italic leading-relaxed text-sm">"{summary}"</p>}
        </div>
        <section><h3 className="text-[12px] font-black uppercase tracking-wider mb-2" style={{ color: theme }}>Professional Qualifications</h3><div className="h-[1px] bg-zinc-200 mb-4"></div><ul className="space-y-2 pl-2 text-[11px] font-medium text-zinc-700">{qualList.map((q, i) => <li key={i} className="flex items-start gap-2"><span className="mt-1.5 h-1.5 w-1.5 rounded-full flex-shrink-0" style={{ backgroundColor: theme }}></span>{q}</li>)}</ul></section>
        <section><h3 className="text-[12px] font-black uppercase tracking-wider mb-2" style={{ color: theme }}>Academic Qualifications</h3><div className="h-[1px] bg-zinc-200 mb-4"></div><EduTable level="Advanced Level" data={cvData.education?.aLevel} /><EduTable level="Ordinary Level" data={cvData.education?.oLevel} /></section>
        <section><h3 className="text-[12px] font-black uppercase tracking-wider mb-2" style={{ color: theme }}>Work Experience</h3><div className="h-[1px] bg-zinc-200 mb-4"></div><div className="space-y-4 pl-1">{workList.map((w: any, i: number) => (<div key={i}><div className="flex justify-between"><h4 className="font-black text-[11px] uppercase">{w.title || w.position}</h4><span className="text-[9px] font-bold text-zinc-400">{w.duration}</span></div><p className="text-[10px] font-bold" style={{ color: theme }}>{w.company}</p><p className="text-zinc-500 mt-1 text-[10px]">{w.description}</p></div>))}</div></section>
        <section><h3 className="text-[12px] font-black uppercase tracking-wider mb-2" style={{ color: theme }}>References</h3><div className="h-[1px] bg-zinc-200 mb-4"></div><div className="grid grid-cols-2 gap-8 pl-1">{refList.map((r: any, i: number) => (<div key={i}><p className="font-black uppercase text-[11px]">{r.name}</p><p className="text-zinc-600 text-[10px] font-medium">{r.designation || r.position}</p><p className="text-zinc-500 text-[10px]">{r.organization || r.company}</p><p className="text-[10px] font-black mt-1" style={{ color: theme }}>Phone: {r.phone}</p></div>))}</div></section>
      </div>
    </div>
  );
    ...
    <div className="h-full min-h-[1120px] bg-white flex flex-col text-[11px]">
      ...
      <div className="flex-1 flex">
        <div className="w-[30%] bg-zinc-50 border-r p-8 space-y-8">
          <section><h3 className="font-black uppercase text-xs mb-3 border-b-2 pb-1" style={{ borderColor: theme }}>Contact</h3><div className="space-y-3 opacity-80 text-[10px]"><p className="flex items-center gap-2"><Phone size={12}/> {phone1}</p><p className="flex items-center gap-2 truncate"><Mail size={12}/> {email}</p><p className="flex items-start gap-2"><MapPin size={12}/> {address}</p></div></section>
          <section><h3 className="font-black uppercase text-xs mb-3 border-b-2 pb-1" style={{ borderColor: theme }}>Details</h3><PersonalInfoList /></section>
          <section><h3 className="font-black uppercase text-xs mb-3 border-b-2 pb-1" style={{ borderColor: theme }}>Skills</h3>{skillsList.map((s, i) => <div key={i} className="mb-1 font-bold text-[9px] uppercase">• {s}</div>)}</section>
          <section><h3 className="font-black uppercase text-xs mb-3 border-b-2 pb-1" style={{ borderColor: theme }}>Languages</h3>{langList.map((l, i) => <div key={i} className="mb-1 font-bold text-[9px] uppercase">• {l}</div>)}</section>
        </div>
        <div className="flex-1 p-10 space-y-8">
          {summary && <p className="text-zinc-600 italic border-l-4 pl-4" style={{ borderColor: theme }}>{summary}</p>}
          <section><h2 className="text-sm font-black uppercase mb-4" style={{ color: theme }}>Professional Qualifications</h2><ul className="list-disc pl-5 space-y-1">{qualList.map((q, i) => <li key={i}>{q}</li>)}</ul></section>
          ...
          <section><h2 className="text-sm font-black uppercase mb-4" style={{ color: theme }}>Work Experience</h2>{workList.map((w: any, i: number) => <div key={i} className="mb-3"><p className="font-black">{w.title || w.position}</p><p className="text-[10px] text-zinc-500">{w.company} | {w.duration}</p></div>)}</section>
          <section><h2 className="text-sm font-black uppercase mb-4" style={{ color: theme }}>References</h2><div className="grid grid-cols-2 gap-4">{refList.map((r: any, i: number) => <div key={i} className="text-[10px]"><b>{r.name}</b><br/>{r.designation}<br/>{r.organization}<br/>{r.phone}</div>)}</div></section>
        </div>
      </div>
    </div>
  );
  ...
  const Layout3 = () => (
    <div className="h-full min-h-[1120px] bg-white p-8 text-[11px]">
      <div className="relative h-32 bg-zinc-900 rounded-3xl flex items-center px-12 mb-16">
        ...
        <div className="absolute -bottom-10 right-12 w-32 h-32 p-1.5 bg-white rounded-full shadow-xl"><img src={profileImg} className="w-full h-full object-cover rounded-full bg-zinc-200" /></div>
      </div>
      <div className="grid grid-cols-12 gap-10">
        <div className="col-span-4 space-y-8">
          <div className="p-6 rounded-3xl border-2" style={{ borderColor: theme }}>
            ...
            <div className="space-y-3 text-[10px]">
              <p><span className="font-bold">Mobile:</span> {phone1}</p>
              <p><span className="font-bold">Email:</span> {email}</p>
              <p><span className="font-bold">Address:</span> {address}</p>
            </div>
          </div>
          <div className="p-6 rounded-3xl bg-zinc-50 space-y-6">
            <div><h3 className="font-black uppercase text-xs mb-3">Info</h3><PersonalInfoList /></div>
        ...
        <div className="col-span-8 space-y-8 pr-4">
          ...
          <section><h2 className="text-sm font-black uppercase border-b-4 inline-block mb-4" style={{ borderColor: theme }}>Professional Qualifications</h2><ul className="list-disc pl-4">{qualList.map((q, i) => <li key={i}>{q}</li>)}</ul></section>
          ...
          <section><h2 className="text-sm font-black uppercase border-b-4 inline-block mb-4" style={{ borderColor: theme }}>Experience</h2>{workList.map((w: any, i: number) => <div key={i} className="mb-2"><p className="font-bold">{w.title || w.position}</p><p className="text-[10px] opacity-60">{w.company} | {w.duration}</p></div>)}</section>
          <section><h2 className="text-sm font-black uppercase border-b-4 inline-block mb-4" style={{ borderColor: theme }}>References</h2><div className="grid grid-cols-2 gap-4">{refList.map((r: any, i: number) => <div key={i} className="text-[10px]"><b>{r.name}</b><br/>{r.designation}<br/>{r.phone}</div>)}</div></section>
        </div>
      </div>
    </div>
  );
  ...
  // ==========================================
  // LAYOUT 4: Executive Minimalist (Dark)
  // ==========================================
  const Layout4 = () => (
    <div className="flex h-full min-h-[1120px] bg-white text-[11px]">
      <div className="w-[30%] bg-zinc-900 text-white p-8 flex flex-col gap-6">
        ...
        <section><h3 className="border-b border-zinc-700 pb-1 mb-3 text-[10px] font-black uppercase text-zinc-400">Contact</h3><div className="space-y-2 text-[9px]"><p>Phone: {phone1}</p><p>Email: {email}</p><p>Address: {address}</p></div></section>
      ...
      <div className="flex-1 p-10 space-y-8">
        ...
        <section><h2 className="text-sm font-black uppercase mb-3 text-zinc-800" style={{ borderBottom: `2px solid ${theme}` }}>Professional Qualifications</h2><ul className="list-square pl-4 text-zinc-600">{qualList.map((q, i) => <li key={i}>{q}</li>)}</ul></section>
  ...
  // ==========================================
  // LAYOUT 5: Corporate Clean Grid (Minimalist Gray)
  // ==========================================
  const Layout5 = () => (
    <div className="h-full min-h-[1120px] bg-white p-12 text-[11px]">
      <div className="flex justify-between items-center border-b-4 pb-8 mb-8" style={{ borderColor: theme }}>
        <div><h1 className="text-4xl font-black uppercase tracking-tighter">{fullName}</h1><p className="text-xs font-bold opacity-50 uppercase tracking-widest mt-2">{phone1} | {email}</p><p className="text-[10px] opacity-50">{address}</p></div>
        <img src={profileImg} className="w-28 h-28 object-cover border-4 border-zinc-100 bg-zinc-200" />
      </div>
      {summary && <p className="text-zinc-600 italic mb-8 border-l-2 pl-4" style={{ borderColor: theme }}>{summary}</p>}
      <div className="grid grid-cols-2 gap-x-12 gap-y-8">
        ...
        <section><h2 className="text-xs font-black uppercase border-b pb-1 mb-4" style={{ color: theme, borderColor: theme }}>Skills & Languages</h2><div className="mb-2"><b>Skills:</b> <div className="flex flex-wrap gap-1 mt-1">{skillsList.map((s, i) => <span key={i} className="bg-zinc-100 px-2 py-0.5 rounded text-[9px]">{s}</span>)}</div></div><div><b>Languages:</b> <div className="flex flex-wrap gap-1 mt-1">{langList.map((l, i) => <span key={i} className="bg-zinc-100 px-2 py-0.5 rounded text-[9px]">{l}</span>)}</div></div></section>
        <section className="col-span-2"><h2 className="text-xs font-black uppercase border-b pb-1 mb-4" style={{ color: theme, borderColor: theme }}>Professional Qualifications</h2><ul className="list-disc pl-4 grid grid-cols-2 gap-2">{qualList.map((q, i) => <li key={i}>{q}</li>)}</ul></section>
        ...
        <section><h2 className="text-xs font-black uppercase border-b pb-1 mb-4" style={{ color: theme, borderColor: theme }}>Experience</h2>{workList.map((w: any, i: number) => <div key={i} className="mb-2"><p className="font-bold">{w.title || w.position}</p><p className="text-[9px]">{w.company} | {w.duration}</p></div>)}</section>
        <section><h2 className="text-xs font-black uppercase border-b pb-1 mb-4" style={{ color: theme, borderColor: theme }}>References</h2><div className="space-y-3">{refList.map((r: any, i: number) => <div key={i} className="text-[10px]"><b>{r.name}</b> ({r.designation})<br/>{r.phone}</div>)}</div></section>
      </div>
    </div>
  );
  ...
  const Layout6 = () => (
    <div className="h-full min-h-[1120px] bg-zinc-50 p-6 text-[11px]">
      <div className="h-full bg-white border border-zinc-200 p-10 relative overflow-hidden">
        ...
        <div className="flex gap-8 items-end mb-10 relative z-10"><img src={profileImg} className="w-32 h-40 object-cover border-4 border-white shadow-xl bg-zinc-200" /><div className="pb-2"><h1 className="text-4xl font-black uppercase tracking-tighter" style={{ color: theme }}>{fullName}</h1></div></div>
        <div className="grid grid-cols-12 gap-10 relative z-10">
          <div className="col-span-7 space-y-8">
            {summary && <p className="italic text-zinc-600">{summary}</p>}
            <section><h2 className="text-sm font-black uppercase border-b-2 pb-1 mb-4" style={{ borderColor: theme }}>Professional Qualifications</h2><ul className="list-disc pl-4">{qualList.map((q, i) => <li key={i}>{q}</li>)}</ul></section>
            ...
            <section><h2 className="text-sm font-black uppercase border-b-2 pb-1 mb-4" style={{ borderColor: theme }}>Work Experience</h2>{workList.map((w: any, i: number) => <div key={i} className="mb-2"><p className="font-bold">{w.title || w.position}</p><p className="text-[10px] text-zinc-500">{w.company} | {w.duration}</p></div>)}</section>
            <section><h2 className="text-sm font-black uppercase border-b-2 pb-1 mb-4" style={{ borderColor: theme }}>References</h2><div className="grid grid-cols-2 gap-4">{refList.map((r: any, i: number) => <div key={i} className="text-[10px]"><b>{r.name}</b><br/>{r.designation}<br/>{r.phone}</div>)}</div></section>
          </div>
          <div className="col-span-5 bg-zinc-50 p-6 rounded-3xl space-y-6">
            ...
            <section><h3 className="font-black uppercase text-xs mb-3">Personal Details</h3><PersonalInfoList /></section>
  ...
  // ==========================================
  // LAYOUT 7: Professional Timeline (Classic Blue)
  // ==========================================
  const Layout7 = () => (
    <div className="h-full min-h-[1120px] bg-white p-12 flex text-[11px]">
      <div className="flex-1 pr-10 border-r-2 border-zinc-100 space-y-8">
        ...
        <section><h2 className="font-black uppercase text-xs mb-4 px-3 py-1 text-white inline-block" style={{ backgroundColor: theme }}>Professional Qualifications</h2><ul className="list-disc pl-5">{qualList.map((q, i) => <li key={i}>{q}</li>)}</ul></section>
        ...
        <section><h2 className="font-black uppercase text-xs mb-4" style={{ color: theme }}>Experience</h2><div className="space-y-4 border-l-2 ml-2 pl-4 border-zinc-200">{workList.map((w: any, i: number) => <div key={i} className="relative"><div className="absolute -left-[21px] top-1.5 w-2.5 h-2.5 rounded-full bg-white border-2" style={{ borderColor: theme }}></div><p className="font-bold">{w.title || w.position}</p><p className="text-[10px] text-zinc-500">{w.company} ({w.duration})</p></div>)}</div></section>
        <section><h2 className="font-black uppercase text-xs mb-4" style={{ color: theme }}>References</h2><div className="grid grid-cols-2 gap-4">{refList.map((r: any, i: number) => <div key={i} className="text-[10px]"><b>{r.name}</b><br/>{r.designation}<br/>{r.phone}</div>)}</div></section>
      </div>
      <div className="w-56 pl-10 space-y-8">
        <img src={profileImg} className="w-full h-48 object-cover rounded shadow-lg bg-zinc-200" />
        <section><h3 className="font-black uppercase text-[10px] mb-3 opacity-50">Contact</h3><div className="space-y-2 text-[10px]"><p><b>Phone:</b><br/>{phone1}</p><p><b>Email:</b><br/>{email}</p><p><b>Address:</b><br/>{address}</p></div></section>
        <section><h3 className="font-black uppercase text-[10px] mb-3 opacity-50">Info</h3><PersonalInfoList /></section>
        ...
        <section><h3 className="font-black uppercase text-[10px] mb-3 opacity-50">Languages</h3>{langList.map((s, i) => <div key={i} className="text-[10px] font-bold mb-1">{s}</div>)}</section>
      </div>
    </div>
  );

  // ==========================================
  // LAYOUT 8: Classic Accountant (Elegant Professional)
  // ==========================================
  const Layout8 = () => (
    <div className="h-full min-h-[1120px] bg-white p-14 text-[11px] leading-relaxed">
      ...
      <div className="space-y-8">
        ...
        <section><h2 className="font-black uppercase bg-zinc-100 px-4 py-1 mb-4 border-l-4" style={{ borderColor: theme }}>Personal Data</h2><div className="px-4"><PersonalInfoList /></div></section>
        <section className="grid grid-cols-2 gap-4 px-4"><div><h2 className="font-black uppercase bg-zinc-100 px-4 py-1 mb-4 border-l-4" style={{ borderColor: theme }}>Skills</h2><ul className="list-disc pl-6">{skillsList.map((s, i) => <li key={i} className="font-bold">{s}</li>)}</ul></div><div><h2 className="font-black uppercase bg-zinc-100 px-4 py-1 mb-4 border-l-4" style={{ borderColor: theme }}>Languages</h2><ul className="list-disc pl-6">{langList.map((s, i) => <li key={i} className="font-bold">{s}</li>)}</ul></div></section>
        ...
        <section><h2 className="font-black uppercase bg-zinc-100 px-4 py-1 mb-4 border-l-4" style={{ borderColor: theme }}>References</h2><div className="grid grid-cols-2 px-4 gap-4">{refList.map((r: any, i: number) => <div key={i}><p className="font-black">{r.name}</p><p>{r.designation}</p><p>{r.organization}</p><p className="font-bold" style={{ color: theme }}>{r.phone}</p></div>)}</div></section>
      </div>
    </div>
  );
