...
export function CVTemplateBase({ cvData, scale = 1 }: { cvData: CVData; scale?: number }) {
  ...
  const summary = pInfo.objective || pInfo.description || (cvData as any).professionalSummary || '';
  ...
  const PersonalInfoList = ({ isDark = false }) => (
    <div className={`space-y-2 pl-1 text-[10px] font-bold uppercase ${isDark ? 'text-zinc-400' : 'text-zinc-600'}`}>
      {nic && <p className="flex flex-col"><span className="text-[8px] opacity-60">NIC:</span> {nic}</p>}
      {dob && <p className="flex flex-col"><span className="text-[8px] opacity-60">DOB:</span> {dob}</p>}
      {gender && <p className="flex flex-col"><span className="text-[8px] opacity-60">Gender:</span> {gender}</p>}
      {nationality && <p className="flex flex-col"><span className="text-[8px] opacity-60">Nationality:</span> {nationality}</p>}
      {religion && <p className="flex flex-col"><span className="text-[8px] opacity-60">Religion:</span> {religion}</p>}
      {civilStatus && <p className="flex flex-col"><span className="text-[8px] opacity-60">Civil Status:</span> {civilStatus}</p>}
    </div>
  );
        ...
        <div className="flex flex-col items-center">
          ...
          <section className="w-full text-left mt-8">
            ...
            <div className="space-y-3 pl-1 text-zinc-600">
              <div className="flex items-center gap-2 text-[10px]"><Phone size={12} style={{ color: theme }}/> {phone1}</div>
              <div className="flex items-center gap-2 truncate text-[10px]"><Mail size={12} style={{ color: theme }}/> {email}</div>
              <div className="flex items-start gap-2 text-[10px]"><MapPin size={12} className="mt-0.5" style={{ color: theme }}/> {address}</div>
            </div>
          </section>
            ...
            <div className="grid grid-cols-2 gap-8 pl-1">
              {refList.map((r: any, i: number) => (
                <div key={i}>
                  ...
                  <p className="text-zinc-600 text-[10px] font-medium">{r.designation || r.position}</p>
                  <p className="text-zinc-500 text-[10px]">{r.organization || r.company}</p>
    ...
    <div className="h-full min-h-[1120px] bg-white flex flex-col text-[11px]">
      ...
      <div className="flex-1 flex">
        <div className="w-[30%] bg-zinc-50 border-r p-8 space-y-8">
          <section><h3 className="font-black uppercase text-xs mb-3 border-b-2 pb-1" style={{ borderColor: theme }}>Contact</h3><div className="space-y-3 opacity-80 text-[10px]"><p>P: {phone1}</p><p>E: {email}</p><p>A: {address}</p></div></section>
        ...
        <div className="flex-1 p-10 space-y-8">
          ...
          <section><h2 className="text-sm font-black uppercase mb-4" style={{ color: theme }}>Qualifications</h2><ul className="list-disc pl-5 space-y-1">{qualList.map((q, i) => <li key={i}>{q}</li>)}</ul></section>
          ...
          <section><h2 className="text-sm font-black uppercase mb-4" style={{ color: theme }}>Experience</h2>{workList.map((w: any, i: number) => <div key={i} className="mb-3"><p className="font-black">{w.title || w.position}</p><p className="text-[10px] text-zinc-500">{w.company} | {w.duration}</p></div>)}</section>
          <section><h2 className="text-sm font-black uppercase mb-4" style={{ color: theme }}>References</h2><div className="grid grid-cols-2">{refList.map((r: any, i: number) => <div key={i} className="text-[10px]"><b>{r.name}</b><br/>{r.phone}</div>)}</div></section>
        </div>
      </div>
    </div>
  );
    ...
    <div className="h-full min-h-[1120px] bg-white p-8 text-[11px]">
      ...
      <div className="grid grid-cols-12 gap-10">
        <div className="col-span-4 space-y-8">
          <div className="p-6 rounded-3xl border-2" style={{ borderColor: theme }}><h3 className="font-black uppercase text-xs mb-4">Contact</h3><div className="space-y-3 text-[10px]"><p>P: {phone1}</p><p>E: {email}</p></div></div>
          <div className="p-6 rounded-3xl bg-zinc-50 space-y-6"><div><h3 className="font-black uppercase text-xs mb-3">Info</h3><PersonalInfoList /></div></div>
        </div>
        <div className="col-span-8 space-y-8 pr-4">
          ...
          <section><h2 className="text-sm font-black uppercase border-b-4 inline-block mb-4" style={{ borderColor: theme }}>Experience</h2>{workList.map((w: any, i: number) => <div key={i} className="mb-2"><b>{w.title}</b><p className="text-[10px] opacity-60">{w.company}</p></div>)}</section>
        </div>
      </div>
    </div>
  );
  ...
  const Layout4 = () => (
    <div className="flex h-full min-h-[1120px] bg-white text-[11px]">
      <div className="w-[30%] bg-zinc-900 text-white p-8 flex flex-col gap-6">
        ...
        <section><h3 className="border-b border-zinc-700 pb-1 mb-3 text-[10px] font-black uppercase text-zinc-400">Contact</h3><p>{phone1}</p><p>{email}</p></section>
      ...
      <div className="flex-1 p-10 space-y-8">
        ...
        <section><h2 className="text-sm font-black uppercase mb-3 text-zinc-800" style={{ borderBottom: `2px solid ${theme}` }}>Experience</h2>{workList.map((w: any, i: number) => <div key={i} className="mb-2"><b>{w.title}</b> - {w.company}</div>)}</section>
      </div>
    </div>
  );
  ...
  const Layout5 = () => (
    <div className="h-full min-h-[1120px] bg-white p-12 text-[11px]">
      ...
      <div className="grid grid-cols-2 gap-x-12 gap-y-8">
        ...
        <section><h2 className="text-xs font-black uppercase border-b pb-1 mb-4" style={{ color: theme, borderColor: theme }}>Experience</h2>{workList.map((w: any, i: number) => <div key={i} className="mb-2"><p className="font-bold">{w.title}</p></div>)}</section>
      </div>
    </div>
  );
  ...
  const Layout6 = () => (
    <div className="h-full min-h-[1120px] bg-zinc-50 p-6 text-[11px]">
      <div className="h-full bg-white border border-zinc-200 p-10 relative overflow-hidden">
        <div className="flex gap-8 items-end mb-10 relative z-10"><img src={profileImg} className="w-32 h-40 object-cover border-4 border-white shadow-xl bg-zinc-200" /><div><h1 className="text-4xl font-black uppercase tracking-tighter" style={{ color: theme }}>{fullName}</h1></div></div>
        <div className="grid grid-cols-12 gap-10 relative z-10">
          <div className="col-span-7 space-y-8"><section><h2 className="text-sm font-black uppercase border-b-2 pb-1 mb-4" style={{ borderColor: theme }}>Education</h2><EduTable level="A/L" data={cvData.education?.aLevel} /><EduTable level="O/L" data={cvData.education?.oLevel} /></section></div>
          <div className="col-span-5 bg-zinc-50 p-6 rounded-3xl space-y-6"><section><h3 className="font-black uppercase text-xs mb-3">Details</h3><PersonalInfoList /></section></div>
        </div>
      </div>
    </div>
  );
  ...
  const Layout7 = () => (
    <div className="h-full min-h-[1120px] bg-white p-12 flex text-[11px]">
      <div className="flex-1 pr-10 border-r-2 border-zinc-100 space-y-8"><header><h1 className="text-4xl font-black uppercase italic" style={{ color: theme }}>{fullName}</h1></header><section><h2 className="font-black uppercase text-xs mb-4" style={{ color: theme }}>Education</h2><EduTable level="A/L" data={cvData.education?.aLevel} /><EduTable level="O/L" data={cvData.education?.oLevel} /></section></div>
      <div className="w-56 pl-10 space-y-8"><img src={profileImg} className="w-full h-48 object-cover rounded shadow-lg bg-zinc-200" /><section><h3 className="font-black uppercase text-[10px] mb-3 opacity-50">Info</h3><PersonalInfoList /></section></div>
    </div>
  );

  const Layout8 = () => (
    <div className="h-full min-h-[1120px] bg-white p-14 text-[11px] leading-relaxed">
      <div className="text-center border-b-2 pb-8 mb-8" style={{ borderColor: theme }}><img src={profileImg} className="w-24 h-24 rounded-full mx-auto mb-4 object-cover border-2 bg-zinc-200" style={{ borderColor: theme }} /><h1 className="text-3xl font-black uppercase tracking-widest">{fullName}</h1></div>
      <div className="space-y-8">
        <section><h2 className="font-black uppercase bg-zinc-100 px-4 py-1 mb-4 border-l-4" style={{ borderColor: theme }}>Personal Data</h2><div className="grid grid-cols-2 gap-y-2 px-4"><PersonalInfoList /></div></section>
        <section><h2 className="font-black uppercase bg-zinc-100 px-4 py-1 mb-4 border-l-4" style={{ borderColor: theme }}>Education</h2><div className="px-4 space-y-4"><EduTable level="A/L" data={cvData.education?.aLevel} /><EduTable level="O/L" data={cvData.education?.oLevel} /></div></section>
      </div>
    </div>
  );
  ...
  return (
    <div id="cv-preview-root" className="w-[210mm] min-h-[297mm] bg-white shadow-2xl mx-auto overflow-hidden origin-top" style={{ transform: `scale(${scale})`, filter: `brightness(${brightness}%)` }}>
      {renderTemplate()}
    </div>
  );
}
