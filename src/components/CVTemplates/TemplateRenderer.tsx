...
export function TemplateRenderer({ cvData }: { cvData: CVData & { manualHeight?: number } }) {
  const themeColor = cvData.customColor || '#1e3a8a';
  // A4 Standard height is 297mm. Custom height applies if manually adjusted.
  const pageHeight = cvData.manualHeight ? `${cvData.manualHeight}mm` : '297mm';
  ...
  const Layout1 = () => (
    <div 
      className="flex h-full bg-white text-zinc-800 w-[210mm] relative overflow-hidden" 
      style={{ minHeight: pageHeight }}
    >
      <div className="w-[35%] bg-slate-800 text-white p-8 flex flex-col gap-8">
        <div className="flex flex-col items-center text-center">
          <img 
            src={cvData.personalInfo?.photo || (cvData as any).profileImage} 
          ...
          <h2 className="text-xl font-bold uppercase leading-tight">
            {cvData.personalInfo?.fullName || (cvData.personalInfo as any)?.name}
          </h2>
        </div>
        <div>
          ...
          <div className="space-y-3 text-[10px] opacity-90">
            <div className="flex items-center gap-2"><Phone size={12}/> {cvData.contact?.phone1 || (cvData.personalInfo as any)?.phone}</div>
            <div className="flex items-center gap-2 truncate"><Mail size={12}/> {cvData.contact?.email || (cvData.personalInfo as any)?.email}</div>
            <div className="flex items-start gap-2"><MapPin size={12} className="mt-0.5"/> {cvData.contact?.address || (cvData.personalInfo as any)?.address}</div>
          </div>
        </div>
        <div>
      ...
      <div className="flex-1 p-10">
        <div className="mb-8">
          ...
          <p className="text-[11px] text-zinc-500 mt-3 italic leading-relaxed">
            {cvData.personalInfo?.description || (cvData.personalInfo as any)?.objective}
          </p>
        </div>
        <div className="space-y-10">
          <section>
            <SectionTitle title="Work Experience" icon={Briefcase} />
            {safeMap(cvData.workExperience || (cvData as any).experience).map((exp: any, i: number) => (
  ...
  const Layout2 = () => (
    <div 
      className="h-full bg-white flex flex-col w-[210mm] relative overflow-hidden" 
      style={{ minHeight: pageHeight }}
    >
      <div className="h-48 p-10 flex justify-between items-center text-white shrink-0" style={{ backgroundColor: themeColor }}>
        <div>
           ...
           <div className="space-y-8">
              <div>
                ...
                <div className="text-[10px] space-y-3 opacity-70">
                   <p className="flex items-center gap-2"><Phone size={12}/> {cvData.contact?.phone1 || (cvData.personalInfo as any)?.phone}</p>
                   <p className="flex items-center gap-2 truncate"><Mail size={12}/> {cvData.contact?.email || (cvData.personalInfo as any)?.email}</p>
                   <p className="flex items-start gap-2"><MapPin size={12}/> {cvData.contact?.address || (cvData.personalInfo as any)?.address}</p>
                </div>
              </div>
              <div>
