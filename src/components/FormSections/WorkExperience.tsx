      ...
      {cvData.workExperience.map((exp) => (
        <div key={exp.id} className="p-6 bg-white border border-zinc-100 rounded-[2.5rem] shadow-xl shadow-zinc-200/50 space-y-4 relative animate-in slide-in-from-bottom-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <select value={exp.startYear} onChange={e => updateWorkExperience(exp.id, { startYear: e.target.value })} className="px-4 py-3 bg-zinc-50 border-none rounded-2xl text-sm font-bold appearance-none cursor-pointer">
              <option value="">Start Year</option>
              {Array.from({ length: 30 }, (_, i) => new Date().getFullYear() - i).map(y => <option key={y} value={y}>{y}</option>)}
            </select>
            <select value={exp.endYear} onChange={e => updateWorkExperience(exp.id, { endYear: e.target.value })} className="px-4 py-3 bg-zinc-50 border-none rounded-2xl text-sm font-bold appearance-none cursor-pointer">
              <option value="">End Year (or Present)</option>
